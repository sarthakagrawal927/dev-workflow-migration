...
- After finishing a task, clean up temporary runtime state you created. Stop dev servers, local agents, watchers, tunnels, and background jobs that are no longer needed. For localhost port cleanup, prefer the installed `ports` CLI to inspect and free ports rather than leaving stale processes running.
